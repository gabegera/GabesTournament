...
void AShooterCharacter::BeginPlay()
{
	...
	if (!InventoryComponent->GetOwnedWeapons().IsEmpty())
	{
		EquipWeapon(InventoryComponent->GetOwnedWeapons()[0].WeaponSlot);
	}
...
APickupActor* AShooterCharacter::FindClosestPickup()
{
	if (PickupSet.Num() <= 0) return nullptr;
	
	TMap<float, APickupActor*> PickupsMap;
	
	float DistanceFromPickup = std::numeric_limits<float>::max();
	for (APickupActor* Pickup : PickupSet)
	{
...
// void AShooterCharacter::ShootProjectile(const TSoftClassPtr<AProjectileActor> ProjectileActor, const float WeaponSpreadInDegrees, const FVector ShotOrigin, const FVector ShotTarget, const float ProjectileVelocity, const float Damage)
// {
...
bool AShooterCharacter::PickupAmmo_Implementation(const EAmmoType AmmoType, const int32 AmmoAmount)
{
...
bool AShooterCharacter::PickupWeapon_Implementation(FWeaponData NewWeapon)
{
...
bool AShooterCharacter::PickupConsumable_Implementation(const EConsumableEffect ConsumableEffect, const int32 ConsumableAmount)
{
