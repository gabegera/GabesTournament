...
AEnemyCharacter* ABaseEnemyAIController::GetEnemyCharacter()
{
	if (!EnemyCharacter)
	{
...
// 	for (int i = 0; i <= GetEquippedWeapon().PelletCount; i++)
// 	{
// 		if (i == 0) i++;
// 		
...
// 			//GetEnemyCharacter()->ShootProjectile(GetEquippedWeapon().ProjectileActor, GetShotSpreadInDegrees(), ShotStart, ShotTarget, GetProjectileVelocity(), GetDamage());
// 		}
// 		
// 		if (GetEquippedWeapon().IsShotgun == false) break;
// 	}
